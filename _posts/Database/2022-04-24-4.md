...
`N+1` 문제란 1번의 쿼리를 사용해서 N개의 데이터를 가져올 때, 해당 데이터와 관련된 모든 데이터를 가져오기 위해 추가적으로 N번의 쿼리가 실행되는 문제를 말합니다. 이를 이해하기 위해서는 먼저 로딩 방식에 대해서 알아야 하므로 간단한 예시를 통해 어떠한 상황인지 파악 후 마지막에 N+1 문제에 대해서 접근해보겠습니다.
